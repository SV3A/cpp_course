...
void printBranches(int branchLen, int nodeSpaceLen, int startLen,
                   int nodesInThisLevel, const deque<Fibtree*>& nodesQueue,
                   ostream& out)
{
  deque<Fibtree*>::const_iterator iter = nodesQueue.begin();
                ...
                ostream& out)
{
  ...
  for (int i = 0; i < nodesInThisLevel; i++, iter++) {
    ...
    out << setw(branchLen+2) << (((*iter) && !((*iter)->empty)) ?
           intToString((*iter)->content) : "");
...
void printLeaves(int indentSpace, int level, int nodesInThisLevel,
                 const deque<Fibtree*>& nodesQueue, ostream& out)
{
  deque<Fibtree*>::const_iterator iter = nodesQueue.begin();
  for (int i = 0; i < nodesInThisLevel; i++, iter++) {
    out << ((i == 0) ? setw(indentSpace+2) : setw(2*level+2)) <<
      ((*iter && !((*iter)->empty)) ? intToString((*iter)->content) : "");
  }
  out << endl;
}

// Pretty formatting of a binary tree to the output stream @ param level
// Control how wide you want the tree to sparse (eg, level 1 has the minimum
// space between nodes, while level 2 has a larger space between nodes)
...
// left margin)
void pretty_print(int level = 1, int indentSpace = 0, ostream& out = cout) {
  ...
  // distance between left neighbor node's right arm and right neighbor node's
  // left arm
  int nodeSpaceLen = 2 + (level+1)*(int)pow(2.0,h);
  // starting space to the first node to print of each level (for the left most
  // node of each level only)
  int startLen = branchLen + (3-level) + indentSpace;

  deque<Fibtree*> nodesQueue;
  ...
  for (int r = 1; r < h; r++) {
    ...
    for (int i = 0; i < nodesInThisLevel; i++) {
      Fibtree *currNode = nodesQueue.front();
